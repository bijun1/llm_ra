...
private:
  ...
  /// Mapping vreg to previous vreg, not original
  std::map<Register, Register> vregmap;

  std::set<Register> rematreg;
...
public:
  ...
  void addVRegMap(Register newreg, Register oldreg) {
    vregmap.insert(std::make_pair(newreg, oldreg));
  }

  void addRematReg(Register reg) {
    rematreg.insert(reg);
  }

  Register getVRegFromMap(Register newreg) const {
    return vregmap.find(newreg)->second;
  }

  bool isRematReg(Register reg) const { return rematreg.find(reg) != rematreg.end(); }
