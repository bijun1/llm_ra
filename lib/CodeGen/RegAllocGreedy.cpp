...
bool RAGreedy::runWithAna(MachineFunction &mf,
                          VirtRegMap& vrm, LiveIntervals& li, LiveRegMatrix& lrm,
                          SlotIndexes& indexes, MachineBlockFrequencyInfo& mbfi,
                          MachineDominatorTree& domtree, MachineOptimizationRemarkEmitterPass& ore,
                          MachineLoopInfo& loops, EdgeBundles& bundles, SpillPlacement& spillplacer,
                          LiveDebugVariables& debugvars, RegAllocEvictionAdvisorAnalysis& evictadvisor,
                          RegAllocPriorityAdvisorAnalysis& priorityadvisor, Spiller* spiller) {
  LLVM_DEBUG(dbgs() << "********** GREEDY REGISTER ALLOCATION **********\n"
                    << "********** Function: " << mf.getName() << '\n');

  MF = &mf;
  TII = MF->getSubtarget().getInstrInfo();
  RegAllocBase::init(vrm, li, lrm);
  Indexes = &indexes;
  Indexes->packIndexes();
  MBFI = &mbfi;
  DomTree = &domtree;
  ...
  ExtraInfo.emplace();
  EvictAdvisor = evictadvisor.getAdvisor(*MF, *this);
  PriorityAdvisor = priorityadvisor.getAdvisor(*MF, *this);
