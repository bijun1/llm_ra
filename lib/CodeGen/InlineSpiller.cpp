...
static cl::opt<bool> NoRemat("no-remat", cl::init(false), cl::Hidden,
                                     cl::desc("Disable inline spill hoisting"));
static cl::opt<bool> NoCoalStack("no-coal-stack", cl::init(false), cl::Hidden,
                                     cl::desc("Disable inline spill hoisting"));
static cl::opt<bool> NoFoldMem("no-fold-mem", cl::init(false), cl::Hidden,
                                     cl::desc("Disable inline spill hoisting"));
static cl::opt<bool> NoHoistBB("no-hoist-bb", cl::init(false), cl::Hidden,
                                     cl::desc("Disable inline spill hoisting"));
...
bool InlineSpiller::hoistSpillInsideBB(LiveInterval &SpillLI,
  ...
  if (NoHoistBB || DefMBB != CopyMI.getParent() || !SrcQ.isKill())
    return false;
...
bool InlineSpiller::reMaterializeFor(LiveInterval &VirtReg, MachineInstr &MI) {
  ...
  Edit->addVRegMap(NewVReg, VirtReg.reg());
  Edit->addRematReg(NewVReg);
...
void InlineSpiller::spillAroundUses(Register Reg) {
  ...
  for (MachineInstr &MI : llvm::make_early_inc_range(MRI.reg_bundles(Reg))) {
    ...
    if (!NoCoalStack && coalesceStackAccess(&MI, Reg))
      continue;
    ...
    if (!NoFoldMem && foldMemoryOperand(Ops))
      continue;

    // Create a new virtual register for spill/fill.
    // FIXME: Infer regclass from instruction alone.
    Register NewVReg = Edit->createFrom(Reg);
    Edit->addVRegMap(NewVReg, Reg);
