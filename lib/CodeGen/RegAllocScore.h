...
public:
  ...
  void dump() {
    std::cout << "Allocation Score " << getScore() << std::endl;
    std::cout << "Copy " << CopyCounts << " ; Load " << LoadCounts << " ; Store " 
    << StoreCounts << " ; ExpRemat " << ExpensiveRematCounts <<  " ; CheapRemat " << CheapRematCounts << std::endl;
  }
};
