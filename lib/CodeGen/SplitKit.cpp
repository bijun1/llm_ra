...
VNInfo *SplitEditor::defFromParent(unsigned RegIdx, const VNInfo *ParentVNI,
                                   ...
                                   MachineBasicBlock::iterator I, bool forbid_remat) {
  SlotIndex Def;
  LiveInterval *LI = &LIS.getInterval(Edit->get(RegIdx));
...
SlotIndex SplitEditor::enterIntvBefore(SlotIndex Idx, bool forbid_remat) {
  ...
  VNInfo *VNI = defFromParent(OpenIdx, ParentVNI, Idx, *MI->getParent(), MI, forbid_remat);
  return VNI->def;
}

SlotIndex SplitEditor::enterIntvAfter(SlotIndex Idx, bool forbid_remat) {
  ...
  VNInfo *VNI = defFromParent(OpenIdx, ParentVNI, Idx, *MI->getParent(),
                              std::next(MachineBasicBlock::iterator(MI)), forbid_remat);
  return VNI->def;
}

SlotIndex SplitEditor::enterIntvAtEnd(MachineBasicBlock &MBB, bool forbid_remat) {
  ...
  VNInfo *VNI = defFromParent(OpenIdx, ParentVNI, Last, MBB,
                              SA.getLastSplitPointIter(&MBB), forbid_remat);
...
SlotIndex SplitEditor::leaveIntvAfter(SlotIndex Idx, bool forbid_remat) {
  ...
  if (SpillMode && !SlotIndex::isSameInstr(ParentVNI->def, Idx) &&
    ...
    defFromParent(0, ParentVNI, Idx, *MI->getParent(), MI, forbid_remat);
    return Idx;
  }
...
SlotIndex SplitEditor::leaveIntvBefore(SlotIndex Idx, bool forbid_remat) {
  ...
  MachineInstr *MI = LIS.getInstructionFromIndex(Idx);
  assert(MI && "No instruction at index");
  VNInfo *VNI = defFromParent(0, ParentVNI, Idx, *MI->getParent(), MI, forbid_remat);
  return VNI->def;
}

SlotIndex SplitEditor::leaveIntvAtTop(MachineBasicBlock &MBB, bool forbid_remat) {
  ...
  VNInfo *VNI = defFromParent(0, ParentVNI, Start, MBB,
                              MBB.SkipPHIsLabelsAndDebug(MBB.begin()), forbid_remat);
...
void SplitEditor::hoistCopies() {
  ...
  for (unsigned i = 0, e = Parent->getNumValNums(); i != e; ++i) {
    ...
    Dom.second = defFromParent(0, ParentVNI, LSP, *Dom.first,
                               SA.getLastSplitPointIter(Dom.first), false)->def;
  }
